...
servers = ["USWest", "USWest3", "USWest4", "USSouthWest", "USSouth3", "USSouth", "USNorthWest", "USMidWest2",
           "USMidWest", "USEast", "USEast2", "EUWest2", "EUWest", "EUSouthWest", "EUNorth", "EUEast", "Australia", "Asia"]
ouputFileError = {
...
def getRealmEyeData(main: str, ign):
    ...
    if 'Fame' in charAlive:
        charAlive = main.split(player)[2].split("Characters")[1].split('Fame')[0]
    elif charAlive == '':
        charAlive = hidden
    try:
    ...
    try:
        ...
        if 'Rank' and 'Account' in bf:
            bf = main.split("Exaltations")[3].split(" ")[0].split("Fame")[0]
    except IndexError:
        try:
            bf = main.split("Exaltations")[3].split(" ")[1].split("Fame")[1]
        except:
            bf = hidden
...
# GetCharacters
def getCharacters(main: str, soup):
    ls = main.split(" ")  # list
    ###### Handle error: 'Characters are hidden'
    try:
        if '<h3>Characters are hidden</h3>' in main:
            return 'Characters are hidden'
        elif '<div class="table-responsive">':
            pass
    except:
        pass
    # TODO return
    ...
    for i in range(len(id)):
        # FIX HERE, View test (Fixed 4:49AM)
        charList.append(ls[ls.index(f'id="{id[i]}"') + 3].split(filter))
    for i in range(len(charList)):
        if charList[i][-1] == "<a":
            # search item and change it for the right one
            for e in range(len(id)):
                fixList.append(
                    ls[ls.index(f'id="{id[e]}"') + 4].split(">")[1].replace("</a", ""))
            item = fixList[i]
            charList[i][6] = item
            charList[i].pop(0)
            charList[i].pop(2)
    ...
    for i in range(len(itemParser)):
        itemList.append(str(itemParser[i][1]).replace(
